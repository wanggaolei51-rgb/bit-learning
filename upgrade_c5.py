...
def main():
    ...
    # ==================== 2. 在topicData末尾追加新话题 ====================
    # topicData结束于 `};`，但需要注意这个是在6509行的那个
    # 找到 topicData 的最后一个条目 ukbi_map 的闭合 },
...
emotions_life: {
  ...
  quiz_answers: "Kunci Jawaban Emosi dan Kehidupan:\\n1.B | 2.B | 3.A | 4.B | 5.B\\n6.B | 7.B | 8.B | 9.B | 10.B\\n\\n Tips: Perhatikan konteks kalimat dan hubungan antar karakter untuk memahami emosi yang tepat."
    ...
    # 在 topicData 的最后一个 }; 之前插入新话题
    # 找到 ukbi_map 的闭合 } 和最后的 };
    # 使用正则找到 topicData 的最后一个条目后的位置
    
    # 更简单的方法：找到 ukbi_map 的最后一个 } 后面跟着 }; 的模式
    pattern = r'(ukbi_map: \{[\s\S]*?\n\},)\n\n\};'
    match = re.search(pattern, content)
    if match:
        insert_pos = match.end(1)
        content = content[:insert_pos] + new_topics + content[insert_pos:]
    else:
        print("ERROR: 无法找到 ukbi_map 的插入位置")
        return
    
    # ==================== 3. 更新 topicCalendar ====================
    # 找到 "2026-08-16": {topic: "ukbi_map"... 这一行并替换
    content = re.sub(
        r'"2026-08-16": \{topic: "ukbi_map", day: 1, label: "🧠 Day 4 · C4_MAP 思维导图复习"\}',
        '"2026-08-16": {topic: "ukbi_exam", day: 1, label: "📜 UKBI 5级 · 真题模拟日"}',
        content
    )
    
    # 在 topicCalendar 的 }; 之前添加新日期
    calendar_new = ''',
  "2026-08-17": {topic: "emotions_life", day: 1, label: "💝 情感及生活 · Day 1"},
  "2026-08-18": {topic: "emotions_life", day: 2, label: "💝 情感及生活 · Day 2"},
  "2026-08-19": {topic: "emotions_life", day: 3, label: "💝 情感及生活 · Day 3"}'''
    
    pattern_cal = r'("2026-08-16": \{topic: "ukbi_exam", day: 1, label: "📜 UKBI 5级 · 真题模拟日"\})\n\};'
    match_cal = re.search(pattern_cal, content)
    if match_cal:
        insert_pos_cal = match_cal.end(1)
        content = content[:insert_pos_cal] + calendar_new + content[insert_pos_cal:]
    else:
        print("ERROR: 无法找到 topicCalendar 的插入位置")
        return
    
    # ==================== 4. 修改 renderTopicDetail 加入 C5 逻辑 ====================
    # 在 const topic = topicData[topicInfo.topic]; 之后插入 C5 复习逻辑
...
'''
    ...
    # 在 renderTopicDetail 的 let html = ``; 之后插入 C5 逻辑
    pattern_c5 = r'(const topic = topicData\[topicInfo\.topic\];\n  const day = topicInfo\.day;\n  let html = ``;)'
    match_c5 = re.search(pattern_c5, content)
    if match_c5:
        insert_pos_c5 = match_c5.end(1)
        content = content[:insert_pos_c5] + '\n' + c5_logic + content[insert_pos_c5:]
    else:
        print("ERROR: 无法找到 renderTopicDetail 的插入位置")
        return
    
    # 在 html += `<div class="card"> 之前插入 reviewHtml
    pattern_card = r'(html \+= `<div class="card"><div class="card-header"><h2>\$\{topic\.icon\} \$\{topic\.title\}</h2><span class="badge">Day \$\{day\}/3)'
    match_card = re.search(pattern_card, content)
    if match_card:
        insert_pos_card = match_card.start(1)
        content = content[:insert_pos_card] + '  html += reviewHtml;\n  ' + content[insert_pos_card:]
    else:
    ...
    print("✅ C4→C5 升级完成")
    print("  - 版本号已更新: V4.17.0")
    print("  - 新话题已添加: ukbi_exam, emotions_life")
    print("  - topicCalendar 已更新")
