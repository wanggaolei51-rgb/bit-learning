...
def main():
    ...
    for i, line in enumerate(lines):
        if 'LEO-BIR V4.16.2-08132026 (UKBI思维导图四级展开)' in line:
            lines[i] = line.replace(
                'LEO-BIR V4.16.2-08132026 (UKBI思维导图四级展开)',
                'LEO-BIR V4.17.0-08162026 (C5课程结构升级)'
            )
            print(f"✅ Title updated at line {i+1}")
        if 'LEO-BIR V4.16.2 · UKBI思维导图四级全部展开 · 语法词汇考试策略详细内容' in line:
            lines[i] = line.replace(
                'LEO-BIR V4.16.2 · UKBI思维导图四级全部展开 · 语法词汇考试策略详细内容',
                'LEO-BIR V4.17.0 · C5课程结构升级 · UKBI真题模拟'
            )
            print(f"✅ Sidebar version updated at line {i+1}")
    
    # ==================== 2. 在 topicData 末尾添加新话题 ====================
    # 找到 topicData 的结束位置（最后一个 };
    # 从文件末尾往前找
    topicdata_end = None
    for i in range(len(lines) - 1, -1, -1):
        if lines[i].strip() == '};' and i > 4926:  # topicData starts at 4926
            # 确认这是 topicData 的结束
            topicdata_end = i
            break
    
    if topicdata_end is None:
        print("ERROR: Cannot find topicData end")
        return
    
    print(f"topicData ends at line {topicdata_end + 1}")
    
    # 在 topicdata_end 之前插入新话题
...
emotions_life: {
  ...
  quiz_answers: "Kunci Jawaban Emosi dan Kehidupan:\\n1.B | 2.B | 3.A | 4.B | 5.B\\n6.B | 7.B | 8.B | 9.B | 10.B\\n\\n Tips: Perhatikan konteks kalimat dan hubungan antar karakter untuk memahami emosi yang tepat."
    ...
    # 在 topicdata_end 行（即 `};` 这一行）之前插入
    new_lines = new_topics.split('\n')
    # 将新话题插入到 topicdata_end 之前
    lines = lines[:topicdata_end] + [l + '\n' for l in new_lines] + ['\n'] + lines[topicdata_end:]
    
    print(f"✅ New topics inserted before line {topicdata_end + 1}")
    
    # ==================== 3. 更新 topicCalendar ====================
    # 替换 2026-08-16 的条目
    for i, line in enumerate(lines):
        if '"2026-08-16": {topic: "ukbi_map", day: 1, label: "🧠 Day 4 · C4_MAP 思维导图复习"}' in line:
            lines[i] = line.replace(
                '"2026-08-16": {topic: "ukbi_map", day: 1, label: "🧠 Day 4 · C4_MAP 思维导图复习"}',
                '"2026-08-16": {topic: "ukbi_exam", day: 1, label: "📜 UKBI 5级 · 真题模拟日"}'
            )
            print(f"✅ 2026-08-16 updated at line {i+1}")
    
    # 找到 topicCalendar 的结束位置
    calendar_end = None
    for i in range(len(lines) - 1, -1, -1):
        if '};' in lines[i] and i > 6510:  # topicCalendar starts around 6511
            calendar_end = i
            break
    
    # 在 calendar_end 之前添加新日期
    new_dates = '''  "2026-08-17": {topic: "emotions_life", day: 1, label: "💝 情感及生活 · Day 1"},
  "2026-08-18": {topic: "emotions_life", day: 2, label: "💝 情感及生活 · Day 2"},
  "2026-08-19": {topic: "emotions_life", day: 3, label: "💝 情感及生活 · Day 3"},'''
    
    if calendar_end:
        new_date_lines = [l + '\n' for l in new_dates.split('\n')]
        lines = lines[:calendar_end] + new_date_lines + lines[calendar_end:]
        print(f"✅ New dates added before line {calendar_end + 1}")
    
    # ==================== 4. 在 renderTopicDetail 中添加 C5 逻辑 ====================
    # 找到 renderTopicDetail 中 "let html = ``;" 的位置
    c5_insert = None
    for i, line in enumerate(lines):
        if 'let html = ``;' in line and i > 6730:  # renderTopicDetail starts at ~6734
            c5_insert = i
            break
    
    if c5_insert is None:
        print("ERROR: Cannot find 'let html = ``;' in renderTopicDetail")
        return
...
'''
    ...
    c5_lines = [l + '\n' for l in c5_logic.split('\n')]
    lines = lines[:c5_insert + 1] + c5_lines + lines[c5_insert + 1:]
    
    print(f"✅ C5 logic inserted after line {c5_insert + 1}")
    
    # 在第一个 html += `<div class="card"> 之前插入 reviewHtml
    card_insert = None
    for i, line in enumerate(lines):
        if 'html += `<div class="card"><div class="card-header"><h2>${topic.icon} ${topic.title}</h2><span class="badge">Day ${day}/3' in line:
            card_insert = i
            break
    
    if card_insert:
        lines[card_insert] = '  html += reviewHtml;\n  html += `<div class="card"><div class="card-header"><h2>${topic.icon} ${topic.title}</h2><span class="badge">Day ${day}/3' + lines[card_insert].split('html += `<div class="card"><div class="card-header"><h2>${topic.icon} ${topic.title}</h2><span class="badge">Day ${day}/3')[1]
        print(f"✅ reviewHtml prepended at line {card_insert + 1}")
    else:
        print("WARN: Cannot find card insertion point, C5 may not display correctly")
    
    # ==================== 5. 写入文件 ====================
    with open('/root/.openclaw/workspace/bit-deploy/index.html', 'w', encoding='utf-8') as f:
        f.writelines(lines)
    
    print("\n🎉 C4→C5 upgrade complete!")
    print("  ✅ Version updated to V4.17.0")
    print("  ✅ ukbi_exam topic added")
    print("  ✅ emotions_life topic added (3 days)")
    print("  ✅ topicCalendar updated")
    print("  ✅ C5 review logic injected into renderTopicDetail")
